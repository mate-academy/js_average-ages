...
function calculateMenAverageAge(people, century) {
  const menOfPeople = people.filter(person => century
...
function calculateWomenAverageAge(people, withChildren) {
  const womenOfPeople = people.filter(person => withChildren
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  ...
  const averageAgeDiff = ageDifference.reduce((sum, motherAge) => {
    return (sum + motherAge);

...
function calculateMenAverageAge(people, century) {
  return getAverage(
    people
      ...
      .filter(man => !century || getCentuary(man) === century)
      .map(person => person.died - person.born)
  );
}

function getCentuary(person) {
...
function calculateWomenAverageAge(people, withChildren) {
  return getAverage(
    people
      ...
      .filter(women => !withChildren || getCildren(people, women).length > 0)
      .map(person => person.died - person.born)
  );
}

function getCildren(people, person) {

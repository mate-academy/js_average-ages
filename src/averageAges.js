...
function calculateMenAverageAge(people, century) {
  const Male = people.filter(person => century ? Math.ceil(person.died / 100)
...
function calculateWomenAverageAge(people, withChildren) {
  const female = people.filter(person => withChildren ? person.sex === 'f'
  && people.some(kid => kid.mother === person.name) : person.sex === 'f');
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  ...
  return kids.map(kid => kid.born - people.find(women => women.name
    === kid.mother).born).reduce((a, b) => a + b) / kids.length;
}

...
function calculateMenAverageAge(people, century) {
  ...
  const callback = (sum, person) => sum + (person.died - person.born);

  return !century
    ? +(filteredPeople.reduce(callback, 0) / filteredPeople.length).toFixed(2)
    : +(filteredByCentury.reduce(callback, 0)
      / filteredByCentury.length).toFixed(2);
}
...
function calculateWomenAverageAge(people, withChildren) {
  ...
  return !withChildren
    ? +(filteredPeople.reduce(callback, 0) / filteredPeople.length).toFixed(2)
    : +(filteredByChild.reduce(callback, 0)
      / filteredByChild.length).toFixed(2);
}

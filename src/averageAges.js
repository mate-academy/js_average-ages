...
function calculateWomenAverageAge(people, withChildren) {
  const women = (withChildren) ? people.filter(woman => woman.sex === 'f'
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  const children = (onlyWithSon)
    ? people.filter(human => human.sex === 'm'
      && people.some(mom => mom.name === human.mother))
    : people.filter(human => people.some(mom => mom.name === human.mother));

  const ageDiff = (children.map(child => child.born - (people.find(mom =>
